        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive-sm">
                    <div class="mb-3 justify-center right" style="float: left; margin-right: 10px;">
                    ...
                    <ul class="pagination pagination-rounded mb-0">
                        @if ($users->currentPage() > 1)
                            <li class="page-item">
                                <button class="page-link"  data-offset="{{ $users->currentPage() - 2 }}">Previous</button>
                            </li>
                        @endif
                        @for ($i = 1; $i <= $users->lastPage(); $i++)
                            <li class="page-item {{ $i == $users->currentPage() ? 'active' : '' }}">
                                <button class="page-link"  data-offset="{{ $i - 1 }}">{{ $i }}</button>
                            </li>
                        @endfor
                        @if ($users->currentPage() < $users->lastPage())
                            <li class="page-item">
                                <button class="page-link" data-offset="{{ $users->currentPage() }}">Next</button>
                            </li>
                        @endif
                    </ul>
                </div>
...
@section('script')
    ...
    <script>
        function getData(offset, limit, search = null) {
            $.ajax({
                type: 'GET',
        ...
        $(document).ready(function() {
            $('.pagination').on('click', 'button.page-link', function(e) {
                ...
                getData(offset, 10, $('#search').val());
            });


            $('#search').on('keyup', function() {
                getData(0, 10, $(this).val());
            });
